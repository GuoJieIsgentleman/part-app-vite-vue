          ...
          <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12" class="mb20">
            <el-form-item label="备件图片">
              <div v-if="state.ruleForm.imgsrc">
                <!-- //邯郸  http://116.132.45.150:6900/ -->
                <el-upload class="avatar-uploader" :multiple="false" ref="upload" :auto-upload="true" :action="'/hg_machineUploadfile?imgid=' +
                  state.ruleForm.id +
                  '&&time1=' +
              ...
              <div v-else>
                <el-upload class="avatar-uploader" :multiple="false" :action="'/hg_machineUploadfile?imgid=' +
                  state.ruleForm.id +
                  '&&time1=' +
                  new Date().getTime().toString()
...
const getusearea = async () => {
  ...
  let { data: res } = await service.get("/hg_getMachineUsearea");
...
const onSubmit = () => {
  ...
  service.post("/hg_updateMachinePart", {
