...
import { getElectronArea, getMachineArea, ListItem, use_proline_options } from "/@/hooks/getHgInfo";
...
const initrepair = async (flag?: any) => {
  let { data: res } = await service.post("/hg_getMahcineRepair", {
    flag: flag,
  });
  if (res != null) {
    state.loading = false;
...
const find = () => {
  ...
  service.post("/hg_getMahcineRepair", {
    flag1: "筛选查询",
    start: state.start,
    end: state.end,
