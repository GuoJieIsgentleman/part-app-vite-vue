...
import { use_proline_options, ListItem, getMachineArea } from "/@/hooks/getHgInfo";
const exportExcel = () => {
...
onMounted(async () => {
  ...
  state.useareas = await getMachineArea()
})
...
const initscrap = async (flag?: any) => {
  let { data: res } = await service.post("/hg_getMchineScrap", {
    flag: flag,
  });
...
const find = () => {
  ...
  service.post("/hg_getMchineScrap", {
    flag1: "筛选查询",
    start: state.start,
    end: state.end,
