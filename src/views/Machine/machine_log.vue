<template>
  <div class="system-menu-container">
    <el-card shadow="hover">
      <el-table id="outTable" v-loading="state.loading" :data="state.partslist" border show align="center" height="600"
        style="width: 100%" header-align="center" max-height="900" fit :row-style="{ height: '10px' }"
        :cell-style="{ padding: '5px 0' }" size="mini" show-summary row-key="id">
        ...
        <el-table-column prop="create_date" label="新增时间" min-width="100" align="center">
        </el-table-column>
        <el-table-column prop="update_date" label="更新时间" min-width="100" align="center">
        </el-table-column>
    ...
    <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
      :page-size="state.pagearray" @prev-click="prev()" @next-click="next()"
...
const handleSizeChange = (val: any) => {
  console.log("每页" + val);
  state.pagesize = val;
  reciveparts(val, state.pagesize);
  //初始化 页数
};
...
const reciveparts = (currentpagecount?: any, pagesize?: any) => {
  let res = service
    .get("/get_log", {
      params: {
    ...
    .then((res) => {
      console.log("机修备件管理res = ", res);

      if (res != null) {
        state.pagecount = res.data.pages;
        state.total = res.data.total;
        state.loading = false;
        state.partslist = res.data.result.map((item: any[]) => {
          return {
            id: item[0],
            username: item[2],
            area: item[3],
