    ...
    <el-card shadow="hover">
      ...
      <el-row :gutter="35">
        <el-col :span="12" :xs="12" :sm="12" :md="12" :lg="6" :xl="6">
          <Auths :value="['btn.add']">
            <el-button @click="onOpenAddMenu()" type="success">添加工装</el-button>
          </Auths>
        </el-col>
...
import AddMenu from "./tooling/addMenu.vue";
import EditMenu from "./tooling/editMenu.vue";
...
const remoteMethod = async (query: string) => {
  ...
  if (query !== '') {
    ...
    setTimeout(async () => {
      loading.value = false;
      let { data } = await service.get("hg_getToolingUsearea", {
        params: {
          query: query
        }
      })
...
const getpartname = async () => {
  ...
  let { data: partname } = await service.get("hg_getToolingPartName", {
    params: {
...
const getspesc = async () => {
  ...
  let { data: type } = await service.get("hg_getToolingSpesc", {
    params: {
...
const selectparts = async () => {
  ...
  let { data } = await service.post("/hg_getToolingParts", {
