  ...
  <div class="system-menu-container">
    <el-card shadow="hover">
      ...
      >
        ...
        <el-table-column prop="spec" label="整改成套备件型号" width="180" align="center">
        </el-table-column>
        ...
        <el-table-column prop="use_count" label="整改领用数量" width="130" align="center">
        </el-table-column>
    ...
    <Equipment_rectification ref="machine_part_equipment_rectification_confirm" />
  </div>
</template>

<script lang="ts" setup>
import Equipment_rectification from "./components/machine_part_equipment_rectification/machine_part_equipment_rectification_confirm.vue";
...
const exportExcel = () => {
  exportTable("#outTable", `${formatDate111(new Date())}成套设备整改明细`);
};
import { formatDate111 } from "/@/utils/formatTime";

const machine_part_equipment_rectification_confirm = ref();

const openscarpuseconfirmref = (row?: any) => {
  machine_part_equipment_rectification_confirm.value.openDialog(row);
  console.log(machine_part_equipment_rectification_confirm);
};
...
const initmachine_part_equipment_rectification = async (flag?: any) => {
  let { data: res } = await service.get("/getmachine_part_equipment_rectification", {
    params: {
      flag: flag,
    },
  });
...
onMounted(initmachine_part_equipment_rectification);
const getuse = () => {
  //未确认记录
  initmachine_part_equipment_rectification("noconfirm");
};

const getalluse = () => {
  //所有记录
  initmachine_part_equipment_rectification("all");
};
...
const find = () => {
  ...
  service
    .get("/getmachine_part_equipment_rectification", {
      params: {
        flag1: "筛选查询",
        start: state.start,
        end: state.end,
