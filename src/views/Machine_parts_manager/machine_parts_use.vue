      ...
      <el-row :gutter="50">
        <el-col :span="12" :xs="12" :sm="12" :md="12" :lg="6" :xl="6">
          <Auths :value="['btn.add', 'btn.other']">
            <el-button size="large" @click="onOpenAddMenu()" type="success"
...
import AdduseRecoredMenu from "./components/machine_parts_userecord/adduseRecoredMenu.vue";
// import AddMenu from "/@/views/parts/components/addMenu.vue";
import EdituseRecored from "./components/machine_parts_userecord/edituseRecored.vue";

import Useconfirm from "./components/machine_parts_userecord/useconfirm.vue";
...
const getusearea = async () => {
  console.log("执行了 getmachine_part_usearea");
...
const inituserecord = async (flag?: any) => {
  console.log(flag);
  let { data: res } = await service.get("/getmachine_parts_userecord", {
    params: {
      flag: flag,
    },
  });
  console.log("getmachine_parts_userecord");
  // console.log(res);
  if (res != null) {
    state.loading = false;
  }
...
const find = () => {
  ...
  service
    .get("/getmachine_parts_userecord", {
      params: {
        flag1: "筛选查询",
        start: state.start,
        end: state.end,
