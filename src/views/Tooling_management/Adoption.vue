...
import AdduseRecoredMenu from "./components/Adoption/tooling_adduseRecoredMenu.vue";
// import AddMenu from "/@/views/parts/components/addMenu.vue";
import EdituseRecored from "./components/Adoption/tooling_edituseRecored.vue";

import Useconfirm from "./components/Adoption/tooling_useconfirm.vue";
...
const exportExcel = () => {
  exportTable("#outTable", `${formatDate111(new Date())}工装领用明细`);
};
...
//初始化领用记录
const initAdoption = async (flag?: any) => {
  console.log(flag);
  let { data: res } = await service.get("/getAdoption", {
    params: {
      flag: flag,
    },
  });
  console.log("getAdoption");
  console.log(res);
  if (res != null) {
    state.loading = false;
  }
...
provide("initAdoption", initAdoption);
onMounted(initAdoption);
