  ...
  <div class="system-menu-container">
    <el-card shadow="hover">
      ...
      <el-pagination
        ...
        v-model:page-size="state.pagearray"
...
const state = reactive({
  ...
  column: [
    ...
    { title: "人员", key: "username", type: "text" },
    { title: "区域", key: "area", type: "text" },
    { title: "型号", key: "spec", type: "text" },
    { title: "名称", key: "item_name", type: "text" },
    { title: "数量", key: "count", type: "text" },
    { title: "创建时间", key: "create_date", type: "text" },
    { title: "更新时间", key: "update_date", type: "text" },
    { title: "操作", key: "flag", type: "text" },
    { title: "类型", key: "remark", type: "text" },
  ],
      ...
      if (res != null) {
        ...
        state.partslist = res.data.map((item: any[]) => {
          return {
            ...
            item_name: item[5],
            count: item[6],
            create_date: formatDate111(item[7]),
            update_date: formatDate111(item[8]),
            flag: item[9],
            remark: item[10],
          };
        });
...
const inittooling_select = () => {
  service
    ...
    .then((res) => {
      console.log("工装jilu名称res = ", res);
      console.log("工装jilu名称res = ", res.data);
      if (res != null) {
        state.ruleForm.part_name = res.data.map((item: any[]) => {
          return {
            value: item[5],
            label: item[5],
          };
        });
        state.ruleForm.usespesc = res.data.map((item: any[]) => {
          return {
            value: item[4],
            label: item[4],
          };
...
const selectparts = async () => {
  let { data: res } = await service.get("/get_log", {
    params: {
  ...
  console.log("工装管理selectparts = ", res);
...
const exportExcel1 = () => {
  ...
  table2excel(state.column, state.partslist, `${formatDate111(new Date())}工装记录.xlsx`);
};
