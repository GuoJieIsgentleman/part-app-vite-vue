...
import EditMenu from "./components/parts/editMenu.vue";
...
const remoteMethod = async (query: string) => {
  ...
  if (query !== '') {
    ...
    setTimeout(async () => {
      loading.value = false;
      let { data } = await service.get("hg_getusearea", {
        params: {
          query: query
        }
      })
...
const getspesc = async () => {
  ...
  let { data: type } = await service.get("/hg_getspesc", {
    params: {
...
const selectparts = async () => {
  ...
  let { data } = await service.post("/hg_getparts", {
