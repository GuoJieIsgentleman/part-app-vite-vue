...
import Confirm from "./components/maintenance/mconfirm.vue";
...
import { ListItem, use_proline_options, getElectronArea } from "/@/hooks/getHgInfo";
...
const getUse_area = async () => { state.useareas = await getElectronArea() }
...
const initmaintenance = async (flag?: any) => {
  let { data: res } = await service.get("/hg_getmaintenance", {
    params: {
      flag: flag,
    },
  });
