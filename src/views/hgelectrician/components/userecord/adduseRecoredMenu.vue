...
import { getElectronSiteUse, getElectronArea, use_proline_options, options, getElectrontype, getElectronSpesc, getElectronPartname, getElectronBalance, ListItem } from '/@/hooks/getHgInfo'
...
const gettype = async () => {
  state.ruleForm.usetype = await getElectrontype(state.ruleForm.select_area)
}

const getpartname = async () => {
  state.ruleForm.part_name = await getElectronPartname(state.ruleForm.select_area, state.ruleForm.select_type)
}

const getspesc = async () => {
  state.ruleForm.usespesc = await getElectronSpesc(state.ruleForm.select_partName, state.ruleForm.select_area, state.ruleForm.select_type)
}

const getbalance = async () => {
  let temp = await getElectronBalance(state.ruleForm.select_area, state.ruleForm.select_type, state.ruleForm.select_spec, state.ruleForm.select_partName)
  ...
  let temp1 = (await getElectronSiteUse(state.ruleForm.select_area, state.ruleForm.select_type, state.ruleForm.select_spec, state.ruleForm.select_partName))
...
const openDialog = async () => {
  ...
  state.ruleForm.userarea = await getElectronArea()
