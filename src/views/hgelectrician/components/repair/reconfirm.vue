...
import { getElectronArea, getElectronBalance, getElectronSiteUse, ListItem, typeOption } from "/@/hooks/getHgInfo";
export default {
  setup() {
    ...
    //初始化 产线 区域
    onMounted(async () => {
      state.ruleForm.areaArrs = await getElectronArea()
    });
    // 打开弹窗
    ...
    const getbalance = async () => {
      let temp = await getElectronBalance(state.partsdetail.area, state.partsdetail.type, state.partsdetail.spec, state.partsdetail.part_name)
