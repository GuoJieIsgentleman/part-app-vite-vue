...
import AdduseRecoredMenu from "./components/userecord/adduseRecoredMenu.vue";
// import AddMenu from "/@/views/parts/components/addMenu.vue";
import EdituseRecored from "./components/userecord/edituseRecored.vue";

import Useconfirm from "./components/userecord/useconfirm.vue";
...
const inituserecord = async (flag?: any) => {
  console.log(flag);
  let { data: res } = await service.get("/getuserecord", {
    params: {
      flag: flag,
    },
  });
  console.log("getuserecord");
  console.log(res);
  if (res != null) {
    state.loading = false;
  }
