    ...
    >
      <el-form :model="ruleForm" size="small" label-width="80px">
        <el-row :gutter="35">
          ...
          <el-col class="mb20" :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
            <el-form-item label="试机人">
              <el-input v-model="ruleForm.name" placeholder="试机人" clearable></el-input>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
...
export default {
  setup() {
    ...
    const onSubmit = async (v: any) => {
      if (v == "") {
      ...
      } else if (state.partsdetail.applicant == "" || state.partsdetail.receipt == "") {
        ElMessage.warning({
          message: "申请人未确认或者收货人未确认",
          type: "warning",
        });
      } else {
        //更新确认人
        ...
        let { data: res } = await service.get("/updaterepair", {
          params: {
            ...
            tryout: v,
            tryoutdate: formatDate111(new Date()),
          },
        });
