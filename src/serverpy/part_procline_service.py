...
def getPart_contrast(machineType: Optional[str] = ""):
    ...
    cursor.execute(f''' 
       ...
       select IFNULL(e.mdtype,e.type) as type,IFNULL(e.part_name,e.machine_name) as name,IFNULL(e.part_spec,e.machine_spesc) as spec,IFNULL(e.kucun,''),IFNULL(e.cx,'') from 
          (
          select * from 
          ( select md.type as mdtype ,md.part_spec,md.part_name,sum(md.balance) as  kucun from part_detail  md  group  by md.part_spec,md.part_name,md.type) a
          left join
          (select mpd.type,mpd.machine_spesc,mpd.machine_name,count(mpd.machine_spesc) as cx from part_procline_detail   mpd group  by mpd.machine_spesc,mpd.machine_name,mpd.type)b
          on a.part_spec=b.machine_spesc and a.part_name=b.machine_name and a.mdtype=b.type
          UNION
          select * from 
          ( select md.type as mdtype,md.part_spec,md.part_name,sum(md.balance) from part_detail  md  group  by md.part_spec,md.part_name,md.type) c
          right join
          (select mpd.type,mpd.machine_spesc,mpd.machine_name,count(mpd.machine_spesc)  from part_procline_detail   mpd group  by mpd.machine_spesc,mpd.machine_name,mpd.type)d
